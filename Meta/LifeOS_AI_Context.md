...
在本地文件系统中，`tasks/需求/Meta` 目录作为「认知对话系统」的落地点，用于承载：
- 人生OS 的 AI 上下文说明（本文件）
- 认知对话系统的最高解释与工作流程说明（`认知对话系统-最高解释文件.md`）
- 单次对话的标准模板与记录（`对话对象-模板.md` 以及具体实例）

AI 在参与与人生OS相关的任何任务时，应优先参考：
- 本文档：理解整体架构与原则；
- `认知对话系统-最高解释文件.md`：理解「我 + Obsidian + AI」这一系统对象的角色与流程；
- 相关领域/项目的「最高解释文件」：理解具体对象的语境与边界。
